(ns tesla_api.main
  "Kotodama WASM entrypoint for the Tesla_api clean-room actor (L4) — Clojure port.
  ...
  facts (`tesla_api.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "tesla_api")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"tesla_api.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "tesla_api-compat" :tier tier :entities entities} 200])
...
(defn start! [] :tesla_api-compat/ready)
